...
class LEOBackgroundGenerator:
    ...
    - Cosmic Photons: Tuerler et al. 2010
...
# Vincent T. (23/11/2023)              
#        Flux = omega*self.TuerlerCosmicPhotons(E)*first*second*third
        Flux = self.TuerlerCosmicPhotons(E)*first*second*third
...
#        fac = 3*cosomega*(1+cosomega)/5*np.pi
        fac = 1-0.4*(1.5*cosomega**2+cosomega**3)
        omega=  2*np.pi*(1-np.cos(np.deg2rad(thetamax)))
#        c = fac*num/den
        c = fac*num/den/omega
#
...
# Vincent T. (23/11/2023): scaling from Sazonov+07 + no scaling for Churazov+08 (reflected CXB)  
...
#        ScalerAbdo = MizunoValue/AbdoValue
        ...
        ScalerChurazovSazonov = 1.

        ChurazovSazonovValue = (self.ChurazovAlbedoPhotons(1850)
                                + self.SazonovAlbedoPhotons(1850))
        MizunoValue = self.MizunoAlbedoPhotons(1850)
        ScalerMizuno = ChurazovSazonovValue/MizunoValue
...
#        return f(E)*redfac
    def PrimaryElectrons(self, E):
        """ From Mizuno+04 with solar mod phi=850MV which provides a good fit to AMS-02 (Aguilar+19)
	    E in keV, Flux in /m2 /s /sr /MeV
            Return a flux in particles /cm2 /s /keV /sr
        """
        filename = './Data/electrons_mizuno_sol850.dat'
        data = pd.read_table(filename, sep='\s+')

        data["Fluxele"] = data["Fluxele"]*10.**(-7.)   # /cm2 /s /keV /sr
...
#        return f(E)*redfac
    def PrimaryPositrons(self, E):
	    ...
	    E in keV, Flux in /m2 /s /sr /MeV
            Return a flux in particles /cm2 /s /keV /sr
        """
        filename = './Data/positrons_mizuno_sol850.dat'
        data = pd.read_table(filename, sep='\s+')

        data["Fluxpos"] = data["Fluxpos"]*10.**(-7.)   # /cm2 /s /keV /sr
...
#        return f(E)*redfac
    ...
    def PrimaryProtons(self, E):
        """ From model ISO-15390 used in CREME2009, mean of min. and max. solar activity spectra
            E in MeV and Flux in /m2 /sr /s /MeV
            Return a flux in particles /cm2 /s /keV /sr
        """
        filename = './Data/protons_creme2009_avgsolar.dat'
        data = pd.read_table(filename, sep='\s+')

        data["Ep"] = data["Ep"]*10.**3.    # keV
        data["Fluxp"] = data["Fluxp"]*10.**(-7.)   # /cm2 /s /keV /sr

        f = self.log_interp1d(data["Ep"].loc[data["Fluxp"] > 0.],
                              data["Fluxp"].loc[data["Fluxp"] > 0.])
...
#        return f(E)*redfac
    def PrimaryAlphas(self, E):
        """ From model ISO-15390 used in CREME2009, mean of min. and max. solar activity spectra
            E in MeV/nucleon and Flux in /m2 /sr /s /(MeV/nucleon)
            Return a flux in particles /cm2 /s /keV /sr
        """
        filename = './Data/alphas_creme2009_avgsolar.dat'
        data = pd.read_table(filename, sep='\s+')

        data["Ea"] = data["Ea"]*4.*10.**3.    # keV
        data["Fluxa"] = data["Fluxa"]/4.*10.**(-7.)   # /cm2 /s /keV /sr

        f = self.log_interp1d(data["Ea"].loc[data["Fluxa"] > 0.],
                              data["Fluxa"].loc[data["Fluxa"] > 0.])
