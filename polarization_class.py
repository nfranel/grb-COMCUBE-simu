...
# class Polarization:
#     """
#     Class to calculate polarization fraction using Monte Carlo method
#     model       : astrophysical model used for the calculation
#     nu_min      : minimum energy over which we integrate in keV
#     nu_max      : maximum energy over which we integrate in keV
#     red_z       : redshift of the source
#     gamma_nu_0  : parameter of our equations defined as gamma * nu_0
#         gamma       : bulk Lorentz factor
#         nu_0        : break frequency of the Band function
#     q           : parameter of our equations defined as theta_v/theta_j
#         theta_v     : observer viewing angle
#         theta_j     : jet opening angle
#     yj          : parameter of our equations defined as (gamma*theta_j)**2
#     alpha       : low energy spectral index of the Band function
#     beta        : high energy spectral index of the Band function
#         beta and alpha are different from the ones from band function, we have :
#         alpha = -(alpha_b +1)
#         beta = -(beta +1)
#
#     integ_steps : number of values used to integrate
#
#     Parameters can only take 4 different shapes :
#         A constant
#         A tuple of len 3 : (valmin, valmax, nval)
#         A tuple of len 2 : ("distri", nval)
...
#                  confidence=None, parallel=True, special_mode=None):
#         if model is None:
#             model = "SO"
...
#                         for theta_nu, ite_theta_nu in arg_convert(self.theta_nu_range):
...
#                                         arg_list.append(var_ite_setting_toma(ite_count, gamma, red_z, theta_j, theta_nu, nu_0, alpha, beta))
#                                         ite_count += 1
#         else:
#             for gamma, ite_gamma in arg_convert(self.gamma_range):
#                 for red_z, ite_red_z in arg_convert(self.red_z_range):
...
#         """
#         if self.model == "SO":
#             return integral_calculation_SO_opti(self.nu_min, self.nu_max, gamma, red_z, theta_j, theta_nu, nu_0, alpha, beta,
#                                                 integ_steps=self.integ_steps, confidence=self.confidence)
#         elif self.model == "SR":
#             return integral_calculation_SR_opti(self.nu_min, self.nu_max, gamma, red_z, theta_j, theta_nu, nu_0, alpha, beta,
#                                                 integ_steps=self.integ_steps, confidence=self.confidence)
#         elif self.model == "CD":
#             return integral_calculation_CD_opti(self.nu_min, self.nu_max, gamma, red_z, theta_j, theta_nu, nu_0, alpha, beta,
#                                                 integ_steps=self.integ_steps, confidence=self.confidence)
#         elif self.model == "PJ":
#             return integral_calculation_PJ(gamma, theta_j, theta_nu)
#
#     def pf_for_multiproc(self, param_list, timer_info=None):
#         """
...
#             if int(param_list[0]) == 1:
#                 est_time = (time.time() - timer_info[0]) * timer_info[1] / 40
#                 if est_time > 3600:
...
#         return [int(param_list[0]), pf_val, pf_error, param_list[1], param_list[2], param_list[3], param_list[4], param_list[5],
#                 param_list[6], param_list[7]]
#
#     def pf_calculation(self, loading_time=True):
#         """
...
#         self.data = VarContainer(numb_val)
#         if loading_time:
#             param_matrix = self.create_params()
#             init_time = time.time()
#             if self.parallel:
#                 with mp.Pool() as pool:
#                     # print(list(zip(param_matrix, repeat([init_time, numb_val]))))
#                     data_to_save = pool.starmap(self.pf_for_multiproc, zip(param_matrix, repeat([init_time, numb_val])))
#                     for data in data_to_save:
#                         self.data.fill_at_ite(data)
#             else:
#                 for param_list in param_matrix:
#                     pf_val, pf_error = self.integral_calculation(param_list[1], param_list[2], param_list[3], param_list[4], param_list[5],
#                                                                  param_list[6], param_list[7])
#                     self.data.fill_at_ite([int(param_list[0]), pf_val, pf_error, param_list[1], param_list[2], param_list[3], param_list[4],
#                                            param_list[5], param_list[6], param_list[7]])
#                     if int(param_list[0]) == 0:
#                         est_time = (time.time() - init_time) * numb_val
#                         if est_time > 3600:
#                             print(f"Estimated running time : {int(est_time / 36) / 100} h")
#                         elif est_time > 60:
#                             print(f"Estimated running time : {int(est_time / 6 * 10) / 100} min")
...
#                             print(f"Estimated running time : {int(est_time)} s")
#                     else:
#                         est_time = (time.time() - init_time) / param_list[0] * numb_val * (1 - param_list[0] / numb_val)
#                         if est_time > 3600:
#                             print(f"Loading time remaining : {int(est_time / 36) / 100} h             ", end='\r')
#                         elif est_time > 60:
#                             print(
#                                 f"Loading time remaining : {int(est_time / 6 * 10) / 100} min             ", end='\r')
#                         else:
#                             print(f"Loading time remaining : {int(est_time)} s             ", end='\r')
#         else:
#             param_matrix = self.create_params()
#             print("Calculation of polarization fractions")
#             if self.parallel:
#                 with mp.Pool() as pool:
#                     data_to_save = pool.starmap(self.pf_for_multiproc, zip(param_matrix))
#                     for data in data_to_save:
#                         self.data.fill_at_ite(data)
#             else:
#                 for param_list in param_matrix:
...
#             pf_error_mat = []
#             q_mat = []
#             for ite in range(len(self.theta_j_range)):
#                 pf_mat.append([])
#                 pf_error_mat.append([])
#                 q_mat.append([])
#             separator = len(self.data.pf)/len(self.theta_j_range)
#             yjvals = []
#             for theta_j_ite in range(len(self.theta_j_range)):
#                 pf_mat[theta_j_ite] = self.data.pf[int(separator * theta_j_ite):int(separator * (theta_j_ite + 1))]
#                 pf_error_mat[theta_j_ite] = self.data.error_pf[int(separator * theta_j_ite):int(separator * (theta_j_ite + 1))]
#                 q_mat[theta_j_ite] = self.data.q[int(separator * theta_j_ite):int(separator * (theta_j_ite + 1))]
#                 yjvals.append(self.data.yj[int(separator * theta_j_ite)])
#             figure, (ax1) = plt.subplots(1, 1, figsize=(10, 6))
#             for yj_index, yj in enumerate(yjvals):
#                 colors = ["blue", "orange", "green", "red"]
#                 colors_err = ["lightblue", "moccasin", "lightgreen", "tomato"]
#                 x_list = np.array(q_mat[yj_index])
#                 y_list = np.array(pf_mat[yj_index])
#                 errors = np.array(pf_error_mat[yj_index])
#                 # Plot normal
#                 # ax1.plot(x_list, y_list, label=f'yj = {yj}', color=colors[yj_index])
#                 # Errorbar
#                 # ax1.errorbar(x_list, y_list, yerr=errors, label=f'yj = {yj}', color=colors[yj_index])
#                 # Zone d'acceptance
#                 ax1.scatter(x_list, y_list, s=2, label=f'yj = {yj}', color=colors[yj_index])
#                 ax1.plot(x_list, y_list - errors, color=colors_err[yj_index])
#                 ax1.plot(x_list, y_list + errors, color=colors_err[yj_index])
#                 ax1.set(xlabel=r'q=$\theta_{\nu}$/$\theta_j$', ylabel='PF',
#                         title=r'Variation of PF as fonction of q=$\theta_{\nu}$/$\theta_j$ using different values of yj',
#                         ylim=(0, 1))
#             ax1.legend()
#             figure.show()
