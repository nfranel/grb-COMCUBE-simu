        ...
        for j in range(args.simulationsperevent):
          ...
          for k, s in enumerate(args.satellites):
            ...
            if theta < s[2]:#source above horizon
              ...
              args.commands.append((not(args.nocosima), not(args.norevan), c.name[i], k, spectrumfile, phtflux, polstr, j, "{:.1f}_{:.1f}".format(np.rad2deg(dec), np.rad2deg(ra)), theta, phi))
    ...
    with open(fname, "w") as f:
      ...
      for line in lines:
        ...
        # Adding 2 more conditions to prevent background sources to be changed
          ...
          f.write("{}.Polarization Absolute 1. {}".format(source, command[6]))
...
def cosirevan(command):
  ...
  if command[0]: #run cosima
    ...
    #print(command, args, pid)
    #print(maketmpsf(command, args, pid))
    #print("cosima -z {0}; rm -f {0}".format(maketmpsf(command, args, pid)))
