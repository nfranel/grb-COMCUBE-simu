...
class GRBSample:
  ...
  def long_distri(self, yscale="log", nbin=50):
    ...
    flux_bin = np.logspace(np.log10(flux_min), np.log10(flux_max), nbin)
    ax5.hist(df_long.PeakFlux, bins=flux_bin, histtype="step", color="blue", label=f"Sample, {n_sample} GRB", weights=[self.sample_weight] * n_sample)
    ax5.hist(df_gbm_long.PeakFlux, bins=flux_bin, histtype="step", color="red", label=f"GBM, {n_gbm} GRB", weights=[self.gbm_weight] * n_gbm)
    ax5.set(title="Peak flux distributions", xlabel="Photon peak flux (photon/cm²/s)", ylabel="Number of GRB", xscale="log", yscale=yscale)
    ax5.legend()
    ax5.grid(True, which='major', linestyle='--', color='black', alpha=0.3)
    ax5.grid(True, which='minor', linestyle=':', color='black', alpha=0.2)
